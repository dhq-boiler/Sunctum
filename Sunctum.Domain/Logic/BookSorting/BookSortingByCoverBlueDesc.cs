...
{
    internal class BookSortingByCoverBlueDesc : IBookSorting
    {
        public IEnumerable<BookViewModel> Sort(IEnumerable<BookViewModel> loadedSource)
        {
            ...
            var sorted = available.OrderByDescending(x => ColorMapLoader.LoadColorMap(x.ID, x.FirstPage.Value.Image.ID, x.FirstPage.Value.Image.Thumbnail.AbsoluteMasterPath, CoverComparator.Color.Blue));
