...
{
    internal class BookSortingByCoverBlueAsc : IBookSorting
    {
        public IEnumerable<BookViewModel> Sort(IEnumerable<BookViewModel> loadedSource)
        {
            ...
            var sorted = available.OrderBy(x => ColorMapLoader.LoadColorMap(x.ID, x.FirstPage.Value.Image.ID, x.FirstPage.Value.Image.Thumbnail.AbsoluteMasterPath, CoverComparator.Color.Blue));
